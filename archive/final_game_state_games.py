...
class CollectiveActionEnv(MultiAgentEnv):
    def __init__(self, return_agent_actions = False, part=False):
        ...
        # environment
        self.observation_space = gym.spaces.Box(-1, self.MAX_CONTRIBUTION + 1, shape=(self.NUM_AGENTS * self.NUM_TURNS,))
    ...
    def find_observations(self):
        values = [np.reshape(self.board, self.NUM_AGENTS * self.NUM_TURNS) for i in range(self.NUM_AGENTS)]
...
saved_competitive_trainer.restore("/home/shatayu/ray_results/MADDPG_CollectiveActionEnv_2021-04-22_14-44-514nj1ccce/checkpoint_50000/checkpoint-50000")

saved_cooperative_trainer = MADDPGTrainer(config=dict(config, **{
        "env": CollectiveActionEnv,
    }), env=CollectiveActionEnv)

saved_cooperative_trainer.restore("/home/shatayu/ray_results/MADDPG_CollectiveActionEnv_2021-04-22_14-44-30pnss4ao7/checkpoint_50000/checkpoint-50000")
...
plt.title('Sum Contributions of Each Agent in Each Round (1000 Games, Game State)')
# plt.savefig('charts/game_state_sum_contributions.png')
